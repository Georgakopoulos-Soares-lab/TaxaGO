        ...
        for line in lines.iter().skip(1) { 
            ...
            if is_combined && fields.len() >= 9 {
                ...
                results.push(GOResult {
                    ...
                    odds_ratio: (odds_ratio * 1000.0).round() / 1000.0, 
                    statistical_significance: fields[4].parse()?,
                    n_study_with_term: Some(fields[5].parse()?),
                    n_study_without_term: Some(fields[6].parse()?),
                    n_background_with_term: Some(fields[7].parse()?),
                    n_background_without_term: Some(fields[8].parse()?),
                    heterogeneity: None,
